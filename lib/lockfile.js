...
function updateLock(file, options) {
    if (typeof options.lockfilePath !== 'string') { throw Error('Options is not defined '); }
...
/* istanbul ignore next */
onExit(() => {
    for (const options in Object.values(locks)) {
        try { options.fs.rmdirSync(options.lockfilePath); } catch (e) { /* Empty */ }
    }
});
